...
после загрузки расширения запрещён — рантайм расширения не переживает fork). Сам мастер
расширение не загружает: это чистый супервизор на `pcntl`/`posix` (оба обязательны,
иначе он бросает `MissingPcntlException`).

Воркеру достаются его stdin/stdout/stderr и ничего больше: иначе `proc_open` оставил
бы открытыми в потомке все дескрипторы мастера — телеметрийные сокеты, соединения
панели, блокировку и журнал, — поэтому спавн направляет их все в `/dev/null`.
...
Такой воркер мастер находит сам и заменяет. Воркер пишет байт в дополнительный
пайп (дескриптор 3, открывается `proc_open` рядом со stdout и stderr) каждый раз,
когда его PHP-поток возвращается в планировщик, но не чаще двух раз в секунду;
мастер читает этот пайп на каждом тике супервизии и засекает последний байт по
своим часам. Перестал возвращаться — перестали приходить байты, это и означает
«завис». Стоит это одного чтения часов на проход, а у воркера без мастера — вообще
ничего.

Так покрыт и цикл обслуживания сервера, и любой цикл на обычном `WaitGroup` — пул
периодических задач, написанный руками консьюмер. Воркер, чей цикл не крутит ни то,
ни другое, может отметиться сам: `Scheduler::get()->markAlive()`, вне
супервизируемого воркера это ничего не делает. По истечении `watchdogTimeoutMs` (по умолчанию 60 с) воркер получает
`SIGTERM`, ещё через `shutdownTimeoutMs` — `SIGKILL`, а слот заполняется так же, как
после любой другой смерти: при `always`, а также при `on-failure`, потому что убитый
воркер завершился не чисто. При `never` слот остаётся пустым, и мастер, у которого не
осталось ни одного живого воркера, завершается:
...
Воркер, зависший на старте, заменяется не сразу, а выждав backoff: порог он
провисел, а не проработал, и если причина общая — подвисший апстрим, к которому
синхронно ходят все воркеры, — замена зависнет так же, и без backoff пул
...
- CPU-bound обработчик при [преемпции](coroutine-switching.ru.md), которую серверы
  взводят по умолчанию: планировщик паркует его корутину и возвращается в цикл, так
  что отметки идут, пока он считает. Всё, что ждёт ввода-вывода, тоже их не
  прерывает, с преемпцией или без: подвешенная корутина возвращает управление в
  цикл, и поллинг простаивающего сервера — тоже: он просыпается каждые 250 мс без
  всякой работы, так что воркер, не обслуживший ни одного запроса, всё равно
  отмечается. Ввод-вывод здесь — это ввод-вывод через SConcur: нативный `sleep`,
  запрос через PDO или вызов `curl` блокируют PHP-поток и не отмечают ничего, ради
  этого случая watchdog и заведён. Отметки прекращает только обработчик, который не
  возвращает управление никому. Если преемпция выключена (`preemptionQuantumMs: 0`), а один обработчик
  всё равно считает дольше порога, этот воркер всё это время не обслуживает ничего и
  будет заменён — поднимите `watchdogTimeoutMs` выше самого длинного такого
  обработчика или поставьте группе `0`.
- Тот единственный воркер, которого сейчас разматывает катящийся `reload`: его
  `SIGTERM` послан и срок на завершение идёт. Слоты, стоящие в очереди за ним, под
  надзором как обычно. Группа, которую дренят после того, как reload убрал её из
  конфига, тоже под надзором, и намеренно: у `retire()` своего срока нет, так что
  зависший воркер иначе держал бы снимаемую группу открытой вечно.
- Воркер, который дренится и никак не додренится: исчерпан `maxRequests` или получен
  `SIGTERM`, а один обработчик ждёт результата, который не придёт. Его цикл
  обслуживания продолжает крутиться, значит он продолжает отмечаться живым, не
  обслуживая ничего. Таким обработчикам нужен `handlerTimeoutMs`; этот случай
  watchdog не видит.
- Воркера, зависшего до того, как он дошёл до цикла обслуживания, — в синхронном
  вызове на этапе бутстрапа. Он ни разу не отметился, а отличить это от скрипта, у
