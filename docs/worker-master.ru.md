...
| `watchdogTimeoutMs` | мастерский (`60000`) | Сколько воркеру позволено не подавать признаков жизни, прежде чем его убьют и заменят — см. [зависший воркер](#зависший-воркер). `0` выключает надзор. |
...
`SIGTERM`, ещё через `shutdownTimeoutMs` — `SIGKILL`, а замену поднимает политика
рестарта:
...
- CPU-bound обработчик при включённой [преемпции](coroutine-switching.ru.md):
  планировщик паркует его корутину и возвращается в цикл, отметки продолжают идти.
  Без взведённой преемпции тот же обработчик неотличим от зависания — и является им
  для всех остальных запросов этого процесса.
