...
Сверх этого мастер дописывает в `argv` воркера свои служебные флаги
`--sconcurMasterPid=<pid>` и `--sconcurWorkerIndex=<0..N-1>` (читаются через
`Worker::masterPid()` и `Worker::index()`); префикс `--sconcur*` исключает коллизии.
