...
скрипта. Это пара к [`SO_REUSEPORT`](http-server.ru.md): воркеры биндят один порт,
ядро балансирует соединения между ними — масштаб HTTP-сервера на все ядра без
внешнего балансировщика. Реализация — в `src/Worker/` (PHP) и универсальный CLI
`bin/sconcur-server`.
...
- [Команды: start / status / stop / reload](#команды-start--status--stop--reload)
- [Параметры](#параметры)
...
# reload — мягкий перекат воркеров по одному (zero-downtime) и ожидание завершения
...
> `runtimeDir`/`name`: `status`/`stop`/`reload` берут их из конфига и так находят
> lock/state/триггер-файлы.

`reload` — мягкий перезапуск воркеров. Команда создаёт триггер-файл
`<runtimeDir>/<name>.reload`; мастер замечает его и по одному перекатывает
...
отклоняется (защита от опечаток вроде `wokerCount`); `name` ограничен набором
`[A-Za-z0-9._-]` (это компонент пути и glob-шаблон ротации); `rotateDays`,
...
- [Сокет-сервер (TCP)](socket-server.ru.md) — ключ `maxConnections` вместо
  `maxRequests` (тот же смысл штатной остановки против утечек); тоже поддерживает
  `reusePort` и `--masterPid`.
...
[`HttpServer(maxRequests: N)`](http-server.ru.md) гасит
...
PDO/`curl`) или в CPU-bound цикл, замораживает единственный PHP-поток воркера —
кооперативная модель его не вытесняет (см. раздел «Таймаут хендлера» в
[доке HTTP-сервера](http-server.ru.md)). `handlerTimeoutMs` на
Go-стороне отдаст клиентам `504`, но сам воркер не завершится — он остаётся
`running` и молча выпадает из обслуживания.

Завершить такой воркер можно только убийством процесса:

- `reload` или `stop` — мастер шлёт `SIGTERM`, ждёт `shutdownTimeoutMs`, затем
  эскалирует в `SIGKILL`; так добивается даже зацикленный воркер (но через грейс).
  Нативный `sleep` обычно снимается уже `SIGTERM` (сигнал прерывает системный вызов),
  CPU-цикл — только `SIGKILL`.
- `kill -9 <pid>` вручную → мастер видит гибель по сигналу и под `always`
  поднимает свежий воркер на замену.

`maxRequests` и сирота-чек тут не помогают: зависший воркер не *завершает* запрос
(счётчик не растёт), а мастер жив.

> Ограничение. Мастер сейчас не детектит «жив, но завис» — видит обычный
> `running` процесс и не трогает его до `reload`/`stop`/ручного `kill`. Автоматическое
> восстановление (watchdog по heartbeat из serve-цикла → `SIGKILL` + respawn при
> устаревании метки) — возможное будущее улучшение, см. планы.
...
Мастер принудительно добавляет `-d display_errors=stderr` в команду каждого воркера
(перед `phpArgs`), поэтому собственный вывод ошибок PHP — синтаксическая ошибка,
незагруженное расширение, фатал по памяти — всегда попадает в журнал с уровнем
`ERROR`, даже когда `php.ini` деплоя ставит `display_errors=Off`. Без этого такой
воркер умирал бы, оставляя после себя только `exited code=255`. Поздний `-d`
побеждает, так что через `phpArgs` это можно осознанно переопределить.
...
протухшего лока, в отличие от PID-файла. Лок-файл намеренно не удаляется (пустой
остаток безвреден).

State-файл `runtimeDir/<name>-state.json` (по умолчанию
`sconcur-server-state.json`) — наблюдаемый флаг: pid, время старта, число
воркеров, путь воркер-скрипта, статус. Пишется атомарно (temp-файл + rename);
удаляется на чистом выходе и остаётся при крахе.
...
2. форвардит `SIGTERM` всем живым воркерам (каждый дренажит свои in-flight сам —
   см. [graceful HTTP-сервера](http-server.ru.md));
