...
вытеснить нечем. (Userland CPU-цикл — другой случай: серверы вытесняют его по
умолчанию, см. [переключение корутин](coroutine-switching.ru.md).)
`handlerTimeoutMs` на Go-стороне отдаст клиентам `504`, но сам воркер не
завершится — он остаётся `running` и молча выпадает из обслуживания.
