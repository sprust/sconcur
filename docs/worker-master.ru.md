...
когда проходит вершину своего цикла обслуживания, но не чаще двух раз в секунду;
мастер читает этот пайп на каждом тике супервизии и засекает последний байт по
своим часам. Перестал крутиться цикл — перестали приходить байты, это и означает
«завис». По истечении `watchdogTimeoutMs` (по умолчанию 60 с) воркер получает
...
Замена поднимается не сразу, а выждав crash-loop backoff. Жертва watchdog по
построению старше порога, так что время жизни ничего не говорит о её здоровье; а если
причина общая — подвисший апстрим, к которому синхронно ходят все воркеры, — замена
зависнет так же, и без backoff пул перезапускал бы себя бесконечно.
...
- Воркера, который ни разу не отметился, — то есть любой скрипт без цикла
  обслуживания. Watchdog действует по свидетельству зависания, а не по его
  отсутствию, так что такой воркер супервизируется ровно как раньше.
- CPU-bound обработчик при [преемпции](coroutine-switching.ru.md), которую серверы
  ...
  отмечается дважды в секунду. Отметки прекращает только
  обработчик, который не возвращает управление никому, — ради него watchdog и
  заведён. Если преемпция выключена (`preemptionQuantumMs: 0`), а один обработчик
...
- Воркера, которого уже разматывает `reload` или `stop`: его `SIGTERM` послан и срок
  на завершение идёт.
