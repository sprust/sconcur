...
Долгоживущий TCP-сервер: сеть живёт в расширении, каждое принятое соединение
...
`id` на стороне расширения через глобальную карту `pendingConnections`).

## Параметры

Конструктор `SocketServer`; дефолты PHP зеркалят дефолты расширения.
