...
получает объект соединения и сам ведёт диалог: читает входящие кадры и пушит кадры
клиенту в любой момент, а не отвечает «одно сообщение — один ответ».
...
| `write(string $data): void` | запушить кадр клиенту (с backpressure: ждёт сброса байтов). Бросает `SocketServerConnectionClosedException`, если соединение порвано |
...
Обработчик не обязан отвечать на каждый входящий кадр и может пушить сколько
...
| `shutdownTimeoutMs` | `10000` | таймаут дренажа активных соединений при остановке |
...
всё же проходит) и возвращает управление. Push-обработчик, который не читает, EOF
не замечает, и его добивает принудительное закрытие после grace-периода
(`drainGrace`, 2 c), а дальше дренаж ограничен `shutdownTimeoutMs`. В пуле
`SO_REUSEPORT` ядро сразу отдаёт новые соединения соседям, и процесс выходит сам.
`reusePort: true` — несколько процессов на одном порту, по процессу на ядро —
...
`config/sconcur.socket-server.config.json`. Мастер разворачивает блок `server`
...
- Broadcast не встроен — пуш в другие соединения на стороне приложения.
