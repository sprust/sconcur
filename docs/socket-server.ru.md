...
безопасно, без экранирования, с естественным лимитом `maxMessageBytes`.
...
| `write(string $data): void` | запушить фрейм клиенту (с backpressure: ждёт флаша). Бросает `ConnectionClosedException`, если соединение разорвано |
...
(`Connection::write` маршрутизируется по `id` на Go-стороне).
...
**полузакрывает на чтение** in-flight соединения: обработчик, читающий в цикле,
получает EOF (его текущая запись ещё проходит) и завершается. Push-only обработчик,
который не читает, добивается принудительным закрытием по истечении грейса. На пуле
`SO_REUSEPORT` ядро тут же раздаёт новые соединения соседям, после чего процесс
завершается сам.
