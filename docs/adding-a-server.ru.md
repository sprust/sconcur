...
2. Лимит на запрос, а не только на сервер. Каждый обработчик ограничен
   ...
   независимо от PHP): до первой записи клиент получает `504`, после начала стрима
   ответ обрывается.
3. Graceful drain и осиротевшие воркеры. Сервер обязан уметь перестать принимать
   новые соединения, не трогая активные (для бесшовной передачи соседям по
   `SO_REUSEPORT`), и самозавершаться, если умер его мастер (`--masterPid`).
...
- `onDrainStart()` — вызывается один раз в начале дренажа: заранее перестать
...
обработчиков в одном цикле ожидания (`waitAnyTimeoutBatch`), переармливает поток
через `next()` и при дренаже штатно гасит флоу (`stopFlow`). Эта механика общая и
переписывать её не нужно.
...
который закрывает только листенер (`http.Server.Shutdown` в отдельной горутине на
фоновом контексте), не отменяя активные запросы. В пуле `SO_REUSEPORT` ядро сразу
отдаёт новые соединения соседям, пока этот процесс дренажит.
...
> `stopFlow`, но тогда он теряет бесшовный дренаж `SO_REUSEPORT` — продакшен-серверу
> под мастером это нужно.
