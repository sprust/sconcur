...
или приложения; ASCII-байты в `BYTEA` работают.

Результаты приходят в бинарном формате PostgreSQL, поэтому колонка читается
только если ядро умеет отрисовать её бинарную форму. Декодируются: целые,
дробные и `NUMERIC` (включая `NaN` и бесконечности — этими же словами), `BOOL`,
`DATE`/`TIMESTAMP`/`TIMESTAMPTZ`, `TIME`, `UUID`, `BYTEA`, `JSON`, `JSONB`,
`TEXT`/`VARCHAR`/`CHAR`/`NAME`, `XML` и перечислимые типы.

Всё остальное — массивы, `INTERVAL`, `INET`/`CIDR`, `MONEY`, `OID`, диапазоны,
составные и геометрические типы — **отклоняется по имени**, с ошибкой, называющей
колонку и тип. Приведите его в запросе (`SELECT tags::text`) и разберите текст на
стороне PHP. Отказ намеренный: эти байты — структура протокола, и отдать их
строкой неотличимо от значения, которое приложение и запрашивало.
