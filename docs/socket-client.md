...
codec is shared with the socket server (`ext/src/socket/`), so a SConcur client
...
plus the write loop, cleaned up on flow stop; `mod.rs` dispatches the
...
`ext/src/socket/`, shared with the socket server.
...
shared `ext/src/socket/` package and the core's own unit tests. The benchmark
