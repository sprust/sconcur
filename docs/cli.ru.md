...
В `composer.lock` версия может лежать с префиксом `v` (`v0.9.0`), поэтому ведущий
`v` срезается и добавляется обратно в URL — иначе получится `vv0.9.0`. В этом слое
...
Код возврата — `0` при `ready=true` и `1` иначе, поэтому пайплайн может ветвиться
без парсинга вывода. На этом построен релизный CI: он гейтит сборку по `ready` и
тегирует релиз как `v<extension_version>`.
