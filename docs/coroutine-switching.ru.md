...
где фибер приостанавливается, пока расширение делает I/O. У CPU-bound кода таких точек
...
таймеру и делает то же самое за вас (в долгоживущих воркерах включена по умолчанию).
...
Долгоживущие воркеры — три сервера (`HttpServer`, `SocketServer`, `WsServer`) и
[консьюмер очередей AMQP](amqp.ru.md#супервизируемый-консьюмер) — на время работы
...
2. Работающий воркер взводит таймер на стороне расширения; каждый квант таймер
...
3. Обработчик расширения, вызванный в PHP-потоке, дёргает preempt-хук
   ...
   `switch()` с истёкшим квантом. По окончании работы таймер снимается.
...
```mermaid
flowchart TB
    Timer["поток-таймер в расширении (каждый квант)"] -->|"EG(vm_interrupt) = true"| Engine["Zend VM (ближайшая граница опкодов)"]
...
Настройка — опция `preemptionQuantumMs` (дефолт `5`, `0` выключает), она есть у всех
четырёх и переопределяется как любая другая опция запуска:
...
CLI-скрипты и использование библиотеки вне этих воркеров таймер не взводят, поэтому там
...
- [HTTP-сервер](http-server.ru.md), [Сокет-сервер](socket-server.ru.md),
  ...
  [AMQP](amqp.ru.md#супервизируемый-консьюмер) — воркеры, включающие преемпцию.
