...
PHP-поток один, и корутины переключаются кооперативно — обычно на вызовах фич, где
фибер приостанавливается, пока Go делает I/O. У CPU-bound кода таких точек нет:
обработчик, молотящий данные, блокирует все остальные корутины процесса, пока не
закончит. Переключение закрывает это в двух видах: `Scheduler::switch()` — явная
точка переключения, которую вы ставите в свои горячие циклы, — и автоматическая
преемпция, когда расширение прерывает VM по таймеру и делает то же самое за вас
(в серверах включена по умолчанию).
...
CPU-bound запрос замораживает всех соседей в полёте на всё своё время, с
переключением их задержка ограничена квантом. Пропускную способность на
CPU-нагрузке по-прежнему даёт per-core пул процессов, поэтому
[вердикт позиционирования](positioning.ru.md#нужен-ли-вам-sconcur) остаётся в силе.
...
Механика: корутина приостанавливается с маркером `PendingSwitchDto`, а планировщик
добавляет её в FIFO-очередь припаркованных. Готовые результаты всегда в приоритете
...
не задерживаются из-за припаркованных «молотилок».
...
обработчиков одновременно в работе на этом воркере. На одном треде это
арифметика, а не настройка.

Замер на 8 воркерах и 256 соединениях, где 90% запросов идут на пустую ручку, а
10% — на обработчик ценой ~49 мс CPU:
...
Вторая ручка — `maxConcurrency` HTTP-сервера: она ограничивает не квант, а число
обработчиков в полёте, то есть саму ширину разделения.
...
Обе ручки двигают систему вдоль одной кривой: низкий p50 для дешёвых запросов и
короткий хвост для тяжёлых одновременно недостижимы. Выбирайте конец под свою
нагрузку — дефолты настроены на дешёвые запросы.
