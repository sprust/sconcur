...
библиотечном коде его никто не взводит, но долгоживущий скрипт может включить
ту же машинерию одним вызовом:

```php
use SConcur\Scheduler\Scheduler;

Scheduler::get()->enablePreemption();   // квант 5 мс по умолчанию
...
таймере прерываний расширения — та же проводка, что у серверов: он паркует
...
прерываются. Повторное включение заменяет предыдущий таймер. Всегда выключайте
в `finally`: таймер продолжает срабатывать до `disablePreemption()` или
завершения процесса.
