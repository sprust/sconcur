...
| `consumers.inFlight` | отдано и ещё не расчитано | реестр в работе |
...
`consumers.avgMs` — по тому, сколько они расчитали; `cpuPercent` — сумма по
процессам и может превышать 100%.

Счётчики консьюмера не стоят ничего лишнего на проводе: доставка считается там, где
уходит в PHP, и расчитывается тем самым `basic.ack` или `basic.nack`, который PHP и
