...
[мастером](worker-master.ru.md). Каждый воркер раз в секунду пушит свой снапшот в
мастер по unix-сокету; мастер держит состояние пула в памяти и отдаёт его на
своём порту — `GET /api/stats`, живая HTML-панель и SSE-поток. Сэмплирование и пуш
живут на Go-стороне воркера; коллектор и панель — чистый PHP в мастере, который
расширение не загружает.
...
Пуш best-effort: нет коллектора (мастер не поднят или перезапускается) — воркер
дропает кадр и продолжает обслуживать трафик. Закрытие соединения означает, что
воркер ушёл, и мастер сразу убирает его из живого пула: никаких файлов и
liveness-проб. Отдельный порт уводит админ-трафик от прикладного и даёт ручку
статистики сокет-серверу, у которого HTTP-маршрутов нет; цикл супервизии мастера
мультиплексирует телеметрийные сокеты через `stream_select` с таймаутом, равным
собственному тику, поэтому под флудом или на застрявшем клиенте деградирует
панель, а не супервизия.
...
сокет- и WebSocket-серверов отдают ту же ручку, только с секцией `connections`
вместо `requests`.
...
- Неверный или отсутствующий токен даёт `404` (а не `401`, чтобы не выдавать
  ручку), как и любой другой путь; не-`GET` метод с валидным токеном — `405`.
...
|---|---|---|
| `panelPort` | порт панели и ручки; нужен вместе с токеном | `0` (выкл.) |
...
|---|---|---|
| `SCONCUR_TELEMETRY_SOCKET` | unix-сокет коллектора; пусто = пуш выключен | пусто |
| `SCONCUR_SERVER_NAME` | имя пула (метка снапшота) | `sconcur-server` |
| `SCONCUR_TELEMETRY_INTERVAL_MS` | период сэмпла и пуша снапшота | `1000` |
...
длительностей не пересекаются: запрос в полёте 7 c попадает только в
...
`snapshotAgeMs` мастер считает по своим часам от момента приёма кадра, поэтому он
не зависит от расхождения часов; живое соединение без свежего снапшота дольше 15 c
помечает воркер как `hung`. Это ловит заклинивший рантайм воркера (застряла сама
горутина-пушер), а не зависший обработчик запроса — пушер независим и шлёт
снапшоты, пока жив Go-рантайм.
