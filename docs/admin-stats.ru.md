...
Соединение у воркера ровно одно, что бы он ни совмещал. Процесс, который
одновременно обслуживает WebSocket-соединения и читает очередь, отдаёт обе секции
в одном снапшоте, а не подключается дважды: коллектор считает воркером
соединение, поэтому второе показало бы один и тот же pid как два воркера с разным
временем старта.
...
- одно соединение на процесс воркера: снапшот несёт все секции, какие у этого
  процесса есть, и один и тот же pid никогда не приходит по двум соединениям.
