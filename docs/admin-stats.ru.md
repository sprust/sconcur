...
работает при заданном `SCONCUR_STATS_DIR` (по умолчанию во временном каталоге).
Те же значения можно задать программно через конструктор `HttpServer` /
`SocketServer` (`adminToken`, `statsDir`, `serverName`, `statsPort`).
...
Ответ HTTP-пула (с секцией `requests`):
