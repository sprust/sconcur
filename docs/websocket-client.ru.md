...
(dial, рукопожатие апгрейда, чтение, запись) живёт в расширении: `connect()`
уходит в задачу рантайма, корутина приостанавливается, поэтому десятки соединений можно
...
останавливается и недочитанное соединение на стороне расширения закрывается (та же
...
дефолты PHP зеркалят дефолты расширения. У долгоживущего соединения нет единого «времени операции»
...
Rust (`ext/src/features/wsclient/`): путь подключения выполняет апгрейд
