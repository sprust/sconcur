...
- Все операции выполняет Rust-драйвер `mongodb` в задаче рантайма; драйвер
  асинхронный, так что конкурентность — рантайма.
- Реестр клиентов (`ext/src/features/mongodb/clients.rs`) — один
  `mongodb::Client` на ключ `uri + serverSelectionTimeoutMs`, общий для всех
  задач с этим ключом. Сам `Client` драйвера уже пулящаяся ручка, поэтому этот
  слой только не даёт двум задачам построить две топологии на один URI. Клиент
  живёт до освобождения расширения, вытеснения по простою нет.
- Курсоры (`ext/src/features/mongodb/states.rs`) — расширение держит курсор
  состоянием и отдаёт батчи по запросу `next`; закрывается при исчерпании, раннем
  выходе или остановке флоу.
- Документы конвертируются между BSON и MessagePack ровно один раз на сообщение,
  на внешней границе (`payloads::decode_params` на входе, кодирование результата
  на выходе). Всё внутри — фильтры, апдейты, стадии пайплайна, разбор bulkWrite,
