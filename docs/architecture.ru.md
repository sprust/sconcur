...
отдельной горутине. Смысл приостановки в этом: корутина никогда не пересекает
границу ради собственной задачи — когда N живых фиберов пересекали границу PHP↔Go
...
стороне Go; именно по флоу расширение отменяет задачи, ждёт их и маршрутизирует
результаты.
...
корутины. Все горутины складывают результаты в один общий буферизованный канал
на стороне Go; `Extension::waitAnyBatch()` блокируется, пока не будет готов
первый результат любого флоу, и в том же cgo-переходе дочитывает все остальные
...
- `FeatureExecutor` — точка входа для фич: определяет async-контекст через
  ...
  задачу той стороне, которая отправит её в Go. На async-пути сам в Go не ходит.
