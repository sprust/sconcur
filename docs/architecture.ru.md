...
корутины. Он крутит точку ожидания `waitAny` (`Extension::waitAnyBatch()`): все
горутины пушат результаты в один общий буферизованный канал на стороне Go; вызов
блокируется до первого готового результата любого флоу и в том же cgo-переходе
дочитывает все остальные уже готовые (до 64). Планировщик разбирает пачку по
одному результату за шаг — по `taskKey` находит нужную корутину и возобновляет
её.
...
5. `WaitGroup::iterate()` (генератор) отдаёт готовые результаты, а пока есть
   ...
   - на верхнем уровне (вне Fiber) крутит `Scheduler::run()` — цикл `waitAny`
     (`Extension::waitAnyBatch()`, готовые результаты дочитываются пачками);
