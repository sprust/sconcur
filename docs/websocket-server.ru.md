...
> CPU-bound / нативный блок. Обработчик, застрявший в нативном вызове (`sleep`,
> синхронный PDO/`curl`), замораживает единственный поток PHP — нативный вызов
> вытеснить нечем. Userland CPU-цикл по умолчанию вытесняется
> ([автоматическая преемпция](coroutine-switching.ru.md), `preemptionQuantumMs`),
> поэтому задерживает соседей лишь на квант. Per-message-таймаута в push-модели нет;
> границы задают idle-таймаут, `writeTimeoutMs`, keepalive-ping и graceful-остановка.
