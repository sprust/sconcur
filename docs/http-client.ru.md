...
[README → Применение](../README.ru.md)).
...
- `sendRequest()` внутри корутины приостанавливает её, не блокируя остальные
  запросы (тот же `Scheduler`, та же точка ожидания `waitAnyBatch`);
...
Транспортная гранулярность — опция `chunkSize` (дефолт 64 KiB): тело ≤ этого размера приходит
...
Ограничение. `download()` игнорирует стриминговое тело запроса
(`streamRequestBody` принудительно выключается, тело буферизуется) — редкий кейс;
обычная загрузка это GET/небольшой POST.
