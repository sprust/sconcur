...
включая ошибочные (`4xx`/`5xx`) и даже оборванные либо прерванные по таймауту:
строка формируется в `finally`, поэтому она пишется при любом исходе, со статусом,
который был (или был бы) отправлен клиенту. Лог всегда включён.

**Запись асинхронная.** PHP только формирует строку и отдаёт её в Go
(`SConcur\Extension\logLine`, fire-and-forget — без флоу/результата); сам вывод
делает фоновая горутина Go (буфер + флаш по таймеру ~100 мс), поэтому
однопоточный цикл сервера **не блокируется** на I/O и не зависит от готовности
читателя `STDOUT` (битый pipe не валит процесс). Если поток логов переполняет
писатель, лишние строки дропаются со счётчиком (для access-лога приемлемо).
