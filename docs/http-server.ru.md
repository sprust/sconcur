...
Воркер — долгоживущий процесс: всё, что создано до `serve()` — бутстрап фреймворка,
DI-контейнер, конфиг, соединения, — строится один раз и переиспользуется каждым
запросом, как в RoadRunner и в отличие от per-request-бутстрапа php-fpm. Обратная
сторона обычная для долгоживущих воркеров: состояние переживает запросы (глобальные
переменные сохраняются, утечки копятся), поэтому request-scoped состояние держите в
[контексте корутины](coroutine-context.ru.md) или локально в обработчике, а процесс
рециклит `maxRequests`.
...
Помимо access-лога сервер пишет в `STDOUT` строки жизненного цикла. При старте — одна
...
по достижению предела [`maxRequests`](#остановка-после-n-запросов). Эти строки пишет
PHP-сторона (в отличие от access-лога) и сразу флашит. Под [мастером
воркеров](worker-master.ru.md) они, как и access-лог, попадают в общий лог.
...
> вызов в обработчике (`sleep()`, синхронный PDO/`curl`, чтение файла) замораживает
> весь сервер — все остальные запросы ждут; уступают управление только асинхронные
> вызовы SConcur. CPU-затратный PHP-код — исключение: по умолчанию сервер вытесняет
> его каждые `preemptionQuantumMs` (см.
> [переключение корутин](coroutine-switching.ru.md)), так что соседей он задерживает
> максимум на квант — но одиночный монолитный внутренний вызов (огромный
> `preg_match`, `json_decode`) по-прежнему непрерываем.
...
Для обработчика, застрявшего в нативном вызове, это спасает только клиента
(корректный код + освобождение соединения и слота `maxConcurrency`), а не сервер:
нативный вызов вытеснить нечем, поэтому обработчик продолжает держать единственный
PHP-поток — все остальные запросы всё это время не обслуживаются (и по дедлайну тоже
отдадут `504`). Userland CPU-цикл — случай мягче:
[автоматическая преемпция](coroutine-switching.ru.md) (включена по умолчанию) паркует
его каждый квант, так что соседи продолжают обслуживаться, просто медленнее. От runaway-
...
Каждый шаг пишется строкой в `STDOUT` — см. [Лог старта и остановки](#лог-старта-и-остановки).
...
| CPU-bound обработчики | ⚠️ ограниченно | Латентность соседей ограничена [автоматической преемпцией](coroutine-switching.ru.md) (включена по умолчанию); throughput по-прежнему даёт per-core пул, а монолитный внутренний вызов непрерываем. |
...
См. также: [README → Принцип работы](../README.ru.md#как-это-работает),
