...
| `shutdownTimeoutMs` | `10000` | Сколько Go ждёт завершения активных соединений при остановке. |
...
- Темп задаёт клиент: чанк подтверждается только после того, как Go фактически
...
асинхронный: фоновая горутина пишет из буфера с флашем по таймеру (~100 мс), при
...
Клиенту отвечает Go-сторона: таймер в `consumeCommands` отдаёт `504`, если к дедлайну
