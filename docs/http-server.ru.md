...
- **Graceful shutdown — на каждый процесс.** Сигнал шлите каждому PID; каждый дренажит
  свои in-flight независимо. Как только процесс закрывает сокет, ядро перестаёт слать
  ему новые соединения, а уже принятые он дослуживает.
