...
| `shutdownTimeoutMs` | `10000` | Сколько Go ждёт дренаж активных соединений при остановке. |
...
их числом в полёте, и `maxConcurrency` ограничивает именно это число. На
профиле из 90% пустых запросов и 10% обработчиков по ~49 мс CPU лимит `4`
уводит p99 с 2.86 с на 659 мс ценой p50, выросшего с 30 до 244 мс. Размен и обе
ручки разобраны в [переключении корутин](coroutine-switching.ru.md#цена-под-нагрузкой).
...
не соединения, и держит к воркерам собственный keep-alive пул. Пустая ручка, 8
воркеров, 256 соединений:
...
меньше CPU на запрос на пустой ручке, 4–9% на БД-ручках — JIT компилирует ровно
тот пер-запросный код оркестрации (цикл планировщика, сборка PSR-7), который
доминирует на PHP-стороне. Ровно так же применимо к socket- и
...
- Финальная запись ответа — fire-and-forget: обработчик завершается, как только
  ...
  сколько drain ждёт дозаписи in-flight соединений. Клиент медленнее этого
  таймаута на самом последнем ответе drain получит его усечённым; если такие
  клиенты важны — поднимите `shutdownTimeoutMs`.
...
sequenceDiagram
    ...
    Go-->>PHP: событие-запрос (батч, HasNext=true; поток качает себя сам)
    ...
    Go->>Client: ответ
    Note over PHP: корутина завершилась сразу после пуша, flow очищается
```
...
Листенер — самокачающийся поток: горутина на Go-стороне публикует каждый
принятый запрос очередным батч-результатом (`HasNext=true`), так что PHP не
платит `next()`-переход на запрос. Backpressure слоёный — буфер общего канала
результатов, канал requests, а дальше блокируется сам `ServeHTTP`. Каждый
запрос обрабатывается в своём flow, поэтому под-задачи обработчика изолированы,
а остановка одного запроса не роняет сервер.
...
команды подтверждаются только после применения — это и есть backpressure
записи. Одноразовая запись `full` — fire-and-forget: корутина завершается сразу
после передачи (ошибка финальной записи и раньше была не видна обработчику), а
сама передача ограничена гвардами соединения (`abandoned`, таймаут
обработчика). На стриминговом пути при отвале соединения или таймауте
обработчик получает ошибку `abandoned` и корректно разворачивается, а не
зависает.
