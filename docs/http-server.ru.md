...
| `readHeaderTimeoutMs` | `10000` | Принимается и не применяется — см. [Таймауты соединения](#таймауты-соединения). |
| `readTimeoutMs` | `30000` | Принимается и не применяется. |
| `writeTimeoutMs` | `30000` | Принимается и не применяется. |
| `idleTimeoutMs` | `60000` | Принимается и не применяется. |
| `shutdownTimeoutMs` | `10000` | Принимается и не применяется. |
...
`0` означает «выключено» для `maxConcurrency`/`handlerTimeoutMs`/`maxRequests`.

### Таймауты соединения

Четыре дедлайна соединения и `shutdownTimeoutMs` пересекают границу, но ни на что
не влияют: расширение ограничивает запрос по `handlerTimeoutMs` и больше ничем.
Медленный клиент поэтому держит своё соединение сколько хочет, а остановка ждёт
обработчиков, а не часов — на практике её ограничивает тот же `handlerTimeoutMs`,
из-за чего оставлять его в `0` на публичном листенере и дорого. Медленных
клиентов обрывайте на обратном прокси впереди.
...
- У ответов 204/304 тело отбрасывается.
...
Клиенту отвечает расширение: задача соединения ждёт первой записи обработчика с
этим сроком и отдаёт `504`, если она не пришла, или обрывает уже начатый стрим.
Это срабатывает независимо от PHP, поэтому клиент получит ответ, даже если
обработчик завис на нативном вызове.
...
Передайте `reusePort: true` каждому процессу на общем порту (на стороне
расширения `SO_REUSEPORT` ставится на сокет до bind,
...
сервер сразу закрывает слушающий сокет — цикл accept останавливается, уже
принятые соединения не отменяются, — дожидается запущенных обработчиков и
выходит.
...
- Финальная запись ответа — fire-and-forget: обработчик завершается, как только
  ответ передан циклу записи соединения, а остановка дальше ждёт дозаписи
  активных соединений, без собственного срока.
...
sequenceDiagram
    ...
    Note over EXT: handle_serve — bind листенера и обслуживание его через hyper
    Note over EXT: состояние сервера и есть обработчик запросов (самокачающийся поток)
    Note over PHP: Scheduler::serve() — цикл waitAnyTimeoutBatch(250ms)
    Client->>EXT: HTTP-запрос
    Note over EXT: serve — захват слота, чтение тела, RequestEvent в канал requests
    ...
    Note over EXT: handle_respond — dispatch write-команды, соединение пишет статус+заголовки+тело
    EXT->>Client: ответ
...
и `serverStates` (`flowKey → serverState` для `StopAccepting`); `server.rs` —
цикл accept и обслуживание запроса поверх `hyper`, отвечающее за семафор
конкурентности, таймаут хендлера, 503/504 и мягкую остановку; `listen.rs` —
...
канала результатов, канал requests, а дальше задача соединения ждёт на отправке. Каждый
...
|---|---|---|
| PHP-FPM | ⚠️ не для этого | Библиотека там работает ([README](../README.ru.md#применение-и-ограничения)), но серверу, слушающему порт, нужен долгоживущий процесс. |
| `pcntl_fork` после загрузки расширения | ✅ да | Потомок пересобирает рантайм при первом же вызове, родителя это не задевает. |
| ZTS-сборка PHP | ❌ нет | Только NTS. |
| TLS / HTTPS | ❌ нет | Только plain TCP; терминируйте TLS впереди (nginx/HAProxy). |
| HTTP/2, WebSocket | ❌ нет | Сервер построен только на HTTP/1.1. WebSocket — [отдельный сервер](websocket-server.ru.md). |
