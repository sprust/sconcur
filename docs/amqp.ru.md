...
**Prefetch больше единицы позволяет двум обработчикам делить канал.** Один
консьюмер — это один канал, и при prefetch N брокер может выдать N его сообщений
сразу, каждое в свой обработчик. Подтверждение доставки делается по её тегу и
безопасно в любом случае, а вот подтверждения публикации живут на *канале*: два
обработчика, одновременно публикующие через `publishConfirmed()` на
`$delivery->channel()`, могут вычитать подтверждения друг друга. При prefetch по
умолчанию, равном единице, на канале всегда один обработчик; выше — обработчику,
публикующему с подтверждениями, стоит взять у соединения свой канал.
