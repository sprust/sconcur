...
потребление живут в расширении; PHP остаётся тонким оркестратором. Внутри
...
брокера; по проводу едут миллисекунды. `0` оставляет стороне расширения её собственное
...
заканчивается, её flow останавливается, и расширение отменяет консьюмера. Та же
...
Расширение никогда не вызывает PHP. Это библиотека, которую вызывает PHP, и любой
переход границы начинается со стороны PHP. То есть консьюмер — это не расширение,
которое нашло работу и толкнуло её слушающему воркеру. Это PHP, который оставил в
расширении незакрытый вопрос и был разбужен, когда расширение смогло на него
ответить.
...
Ожидание происходит в расширении, на задаче рантайма, которая исполняет `next`, а не в PHP,
...
sequenceDiagram
    ...
    participant G as Задача консьюмера
    ...
    Note over G,B: задача рантайма ждёт на канале доставок драйвера
    B-->>G: доставка
    G-->>S: результат
...
## Соединения и каналы на стороне расширения
...
Каждый процесс держит свой рантайм расширения и свой пул соединений; между ними ничего
не разделяется.
...
Это сервер во всём, кроме сокета. Расширение открывает консьюмеров и публикует
...
Каналы за этими консьюмерами принадлежат стороне расширения, и именно это делает остановку
...
### Подтверждение, которое не удалось отправить

То же переоткрытие закрывает сбой, которого брокер не видит. Доставка, подтверждение
которой не ушло, — канал, на котором она пришла, исчез из-под обработчика, команду
отклонили — всё ещё числится за брокером, а брокер об этом не слышал: он продолжает
считать сообщение занимающим prefetch этого консьюмера. При `prefetchCount: 1` консьюмеру
больше ничего не шлют, и очередь стоит, пока не истечёт брокерский `consumer_timeout` —
по умолчанию полчаса.

Знает об этом только воркер, и он сразу об этом сообщает. Сбой пишется в журнал,
консьюмер отменяется, его канал отдаётся — что возвращает сообщение, — и через секунду
очередь берёт свежий консьюмер:
...
Сообщение приходит снова и делается заново — это тот же at-least-once, который AMQP даёт
и без того. Так обрабатывается любой сбой подтверждения, не только исчезнувший канал:
каждый из них оставляет сообщение должным и стоит того же ожидания, а какой именно это был
сбой, говорит записанное исключение. Воркер, который дренажится, не трогают: его
консьюмеров отменяют насовсем.
...
расширение его уже освободило. Чтобы продолжить, откройте новый канал —
...
| `pconnect()`, `pdisconnect()` | — | — persistent-соединения это понятие php-fpm; пул на стороне расширения и так переживает PHP-объект |
...
- `Timestamp` от 2^63 и выше отклоняется при публикации. AMQP считает
  беззнаковые 64-битные секунды, но ни PHP-int, ни метка времени, из которой строится
...
пересекает границу, а перекрывать нечего. `basic.get` брокера ждёт, и
