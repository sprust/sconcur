...
возьмёт нового сообщения, пока не вернётся её обработчик, и graceful-дренаж будет
ждать её до `drainTimeoutMs`.
...
попытки, на канале самой доставки.
...
ответа становится кодом исключения. Исключения — последние пять строк таблицы:
...
Последние пять — `LogicException`, а не `AmqpException`: ничего не отправлялось, брокера
