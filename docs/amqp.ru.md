...
бросает `QueueException` («Consumer timeout exceed») вместо завершения цикла — воркер, принявший тишину за
...
| `maxMessages` | `0` (без предела) | Дренировать и выйти после стольких сообщений. Это бюджет, а не точный счёт: корутины, уже вошедшие в обработчик, дорабатывают своё сообщение, так что пул из N может уйти на N-1 сверх лимита. |
...
| `maxReconnectAttempts` | `10` | Сколько раз подряд консьюмер, которого забрал брокер, переоткрывает свою очередь — с паузой в секунду — прежде чем от неё отказаться. `0` завершает его на первом же сбое. |
| `preemptionQuantumMs` | `5` (`0` — выкл.) | Квант автоматической преемпции на время потребления. Именно она позволяет `handlerTimeoutMs` и остановке достать обработчик, занятый вычислением, — см. [переключение корутин](coroutine-switching.ru.md). |
...
Консьюмера завершает не только удаление очереди: канал умер из-за чужого `404`, нода
кластера переключилась, истёк `readTimeoutSeconds` соединения. Корутина через секунду
переоткрывает свою очередь на свежем канале и сообщает об этом:

```
consumer: orders lost (…: Consumer sconcur-ctag-7 was cancelled by the broker.); reopening in 1000ms, attempt 1 of 10
```

После `maxReconnectAttempts` попыток подряд она от очереди отказывается. Удалённая
очередь отвечает `404` вечно, и консьюмер, который пробует бесконечно, держал бы воркер
живым в виде пула с мёртвыми очередями. Консьюмер, проработавший дольше паузы между
попытками, получает свежий бюджет — сбой через час не засчитывается к прошлому.

Двух вещей он не делает. Не переоткрывает **соединение**: оно общее для всех корутин
воркера, и закрытие его из одной унесло бы каналы всех остальных, — поэтому умершее
соединение по очереди валит всех консьюмеров, воркер выходит ненулевым кодом, а
[мастер](worker-master.ru.md) поднимает свежий процесс. И не переподтверждает сообщение,
бывшее в работе: оно не было подтверждено, поэтому брокер отдаст его снова сам.

Когда от своих очередей отказались все консьюмеры, `consume()` бросает то, что завершило
первого, — воркер выходит, а не остаётся сидеть без чего бы то ни было тянуть.
...
| брокер ответил кодом уровня соединения (5xx) или соединение умерло | `ConnectionException` | код ответа; соединение, оборвавшееся посреди кадра, отдаёт собственный `501` драйвера, и только сбой, которому кода не поставил никто, отдаёт 0 |
...
`ConnectionException`, и то же самое делает консьюмер, чей поток оно унесло.
