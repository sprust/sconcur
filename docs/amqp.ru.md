...
соединение; тот, чей канал уже был мёртв к моменту вызова, — эти сообщения никто не
смотрел, поэтому они возвращаются в очередь независимо от настройки. Политика выше —
про упавшие задачи. Публикация, которую брокер отверг с кодом ответа — 404 в
несуществующий обменник, — это как раз решение, и она следует политике: иначе за тем
же сообщением пришли бы снова и снова.
