...
$delivery->reject();                 // как nack(requeue: false), ровно одно сообщение
```
...
`nack` и `reject` — один и тот же отказ AMQP, и различаются они двумя вещами:
значением по умолчанию и пакетной формой. `nack` возвращает сообщение в очередь,
если не сказали обратного, а `multiple: true` отказывается разом от всего, что было
до него на канале; `reject` отказывается ровно от одного сообщения и не возвращает
его, пока не попросят. Отказ без возврата уходит туда, куда его отправит очередь: в
названный ею dead-letter-обменник — или никуда, если она его не называет.
...
переоткрывает свою очередь на свежем канале — и делает это, пока переоткрытие вообще
может сработать, — сообщая о каждой попытке:
...
всё время своей жизни, по строке в секунду на каждую корутину — и это намеренно: очередь,
которая вернулась, подхватывается без вмешательства, а та, которая не вернулась, видна в
журнале, а не молча не читается.

Насовсем консьюмера завершает уход **соединения**. Оно общее для всех корутин воркера, и
закрытие его из одной унесло бы каналы всех остальных, поэтому оно не переоткрывается: все
консьюмеры завершаются по очереди, `consume()` бросает то, что завершило первого, воркер
...
ответа становится кодом исключения. Исключения — последние три строки таблицы:
...
Последние три — `LogicException`, а не `AmqpException`: ничего не отправлялось, брокера
