#!/usr/bin/env bash
#
# RoadRunner counterpart of load-stats.sh: the same load + resource harness,
...
#
# Same honesty rules as load-stats.sh: the rr process (workers inherit its
...
# comparable with a load-stats.sh run at the same WORKERS/SERVERS count.
#
# Run from the HOST (wrk + docker live there); the server runs in the container.
#
# Usage:
#   tests/benchmarks/http/rr-load-stats.sh
#   WORKERS=8 CONNECTIONS=256 DURATION=20 tests/benchmarks/http/rr-load-stats.sh
#
