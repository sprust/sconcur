...
- [Мастер воркеров](docs/worker-master.ru.md) — супервизор пула процессов-воркеров
  ...
  воркеров.
- [Админ-статистика сервера](docs/admin-stats.ru.md) — ручка
  `GET /sconcur-server-api/admin/stats`: агрегированная статистика по пулу
  `SO_REUSEPORT` целиком на стороне Go (снапшоты воркеров, Bearer-токен, метрики).
