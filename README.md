...
> ⚠️ Экспериментальный проект, пока не готов к продакшену. Очередная попытка
> сделать PHP асинхронным, но без расширения на C: на Go — языке, который всё
> шире распространяется среди PHP-разработчиков.
...
- Простота добавления фич: чтобы подключить новую I/O-операцию, достаточно
  ...
  Блокирующий драйвер (например, официальный MongoDB) используется как есть.
...
- Не вызывайте `exit()`/`die()` при активных задачах. Если оборвать процесс, пока
  в Go выполняются задачи (незавершённый `WaitGroup`, открытый курсор), поведение
  не определено: результаты и освобождение ресурсов не гарантируются. Сначала
  доведите задачи до конца или остановите их (`WaitGroup::stop()`), затем
  завершайте процесс.
...
- [Мастер воркеров](docs/worker-master.ru.md) — супервизор пула процессов-воркеров
  ...
  воркеров.
- [Статистика сервера](docs/admin-stats.ru.md) — отдельный stats-сервер на своём
  порту, ручка `GET /api/stats`: агрегированная статистика по пулу `SO_REUSEPORT`
  (HTTP или socket) целиком на стороне Go (снапшоты воркеров, Bearer-токен, метрики).
...
- Лимит параллелизма группы — `WaitGroup::create(maxConcurrency: N)`, backpressure
  для пула задач.
