...
- Только CLI (SAPI `cli`) — это про SAPI, а не про «без веба». Библиотека
  рассчитана на долгоживущие CLI-процессы: воркеры, демоны, консольные команды —
  и сами HTTP-, WebSocket- и сокет-серверы, которые являются обычными
  PHP-скриптами, запущенными из консоли и самостоятельно слушающими порт (FPM в
  цепочке не участвует, модель как у RoadRunner / Swoole / ReactPHP). С PHP-FPM и
  mod_php использовать нельзя: расширение держит Go-рантайм и горутины на уровне
  процесса, а модель FPM (короткий запрос-ответ, общие процессы пула) этому
  противоречит.
