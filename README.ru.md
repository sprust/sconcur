...
(`waitAnyBatch`): блокируется до первого готового результата любого флоу,
вместе с ним забирает за тот же переход пачку уже готовых и возобновляет нужные
корутины по `taskKey`. Результаты приходят в порядке завершения задач, а не в
порядке `add()`.
...
- Минимальный и стабильный C-интерфейс клея. Граница PHP↔Go — горстка обобщённых
  cgo-функций (`push`, `wait`, `waitAny`/`waitAnyBatch`, `next`, `stopFlow` плюс жизненный цикл
...
- [Переключение корутин](docs/coroutine-switching.ru.md) — `Scheduler::switch()`
  для CPU-bound циклов и автоматическая преемпция серверов: механика, квант,
  предохранители, пределы.
...
- [Бенчмарки фич](docs/benchmarks.ru.md) — замеры по каждой фиче (native/sync/async):
  цена границы PHP↔Go, выигрыш конкурентного веера на дисковых БД, таблицы метрик.
...
- Оптимизация выполнения в синхронном режиме — вызов вне корутины идёт в Go
  ...
  синхронного пути.
- Попробовать кросс-процессный режим конкурентности — чтобы расширение держало и
  распределяло конкурентность по нескольким процессам, а не только внутри одного,
  и веер мог задействовать несколько процессов (и ядер), а не горутины одного
  процесса.
