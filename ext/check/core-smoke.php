...
/**
 * Smoke check for the Rust core: the parts of it every feature leans on,
 * exercised through the unmodified PHP package. It runs before the suites do
 * (`make ext-check`), so a core that cannot answer at all is reported as that
 * rather than as several hundred failing tests.
 *
 * Run inside the `php` container:
 *   php -d extension=/sconcur/ext/build/sconcur.so ext/check/core-smoke.php
 *
