...
use crate::features::httpserver::requeststats::RequestStats;
...
/// into its body stream.
struct PendingGuard {
    registries: &'static Registries,
    request_id: String,
    /// None when no collector is configured: an unwatched server records nothing
    /// and pays no lock per request.
...
impl Drop for PendingGuard {
    fn drop(&mut self) {
        ...
        if let Some(stats) = &self.stats {
            stats.request_ended(&self.request_id, self.started);
        }
    }
}
...
async fn serve_request(
    ...
    if let Some(stats) = &stats {
        stats.request_began(&request_id, started);
    }
