...
16. [~] Тесты добавлены: **413** (тело сверх лимита) и приём тела под лимитом
    (`HttpServerErrorsTest`); **graceful drain** — `HttpServerGracefulShutdownTest`
    поднимает сервер отдельным процессом, шлёт `SIGTERM` во время in-flight `/sleep`,
    проверяет дослуживание (200) и чистый выход (код 0). **Осталось:** бинарное тело;
    вынос логики `serve()` в чистую юнит-тестируемую единицу.
