...
- **Готово (access-лог):** встроенный — после каждого запроса `HttpServer` пишет
  ...
  Запись **асинхронная**: PHP формирует строку и отдаёт её в Go через cgo-экспорт
  `logLine` (fire-and-forget, без флоу), а пишет фоновая горутина Go (пакет
  `internal/logger`: буфер + флаш по таймеру + дроп при переполнении), поэтому
  однопоточный цикл не блокируется на I/O. (Ранее был колбэк `accessLog`/
  `AccessLogEntry`, затем синхронный `fwrite`+`fflush` — оба заменены.)
