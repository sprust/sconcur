...
5. [x] **Тайм-аут хендлера.** `HttpServer(handlerTimeoutMs: N)` (0 = выкл). Go ограничивает
   время до первого ответа; по дедлайну → `504` + освобождение слота/соединения. Дедлайн
   покрывает только время до старта ответа (начатый стрим не рвётся). Повисший respond
   разблокируется сигналом `abandoned` (корутина разворачивается, без залипания goroutine).
   Покрыто Go-тестом `TestServeHTTPHandlerTimeout` и `HttpServerHandlerTimeoutTest`.
