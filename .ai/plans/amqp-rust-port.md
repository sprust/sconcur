...
> **Статус: сделано (2026-09-02).** Фича перенесена, подпорки сняты: `make test`
> гоняет всё дерево, релиз публикует Rust-сборку, консьюмер-мастер и
> `make mem-leak-amqp` смотрят на `ext/`. Go-ядро осталось эталоном, но зелёным
> его больше никто не держит — CI его не трогает.
>
