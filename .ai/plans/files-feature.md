...
- Кросс-девайсный `move` идёт через временный файл рядом с назначением, а не копирует
  ...
  файл.
- Размер blocking-пула в итоге не трогаем: план предлагал задать его явно, но пул
  принадлежит процессу, а не фиче, и его делит разрешение имён. Явно задаём то же
  число, что и tokio (512), только чтобы оно было записано, а не подразумевалось.
- Тесты писателя и стримов опираются на `/dev/full`: он отвечает ENOSPC на каждый байт,
  и это единственный детерминированный способ получить сбойный чанк — гонка с дедлайном
  давала тесты, которые проходили при удалённой правке.
