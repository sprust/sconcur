...
| RemoveDirectory | `rmd` | `Files::removeDirectory(path, recursive)` | Удаление |
...
| ReadLines | `rdl` | `Files::readLines(path, batchSize)` | `Iterator<int, string>` — строками |
| ListStream | `lst` | `Files::walk(path, pattern)` | `Iterator<int, DirectoryEntry>` — обход дерева батчами |
| WriteOpen | `wro` | `Files::openWriter(path, mode, permissions)` | Открывает писателя, отдаёт `Dto\FileWriter` |
...
Писатель повторяет upload-сессии `HttpClient` (`ext/src/features/httpclient/mod.rs`,
`UploadSession`): `WriteOpen` регистрирует в `Registries` запись «канал чанков +
место под причину смерти» и заводит состояние через `states::get().register_with_flow()`,
`WriteChunk` шлёт чанк в `mpsc` (ожидание на `send` и есть ожидание, пока расширение
успеет записать), `WriteClose` закрывает канал и забирает итог. Идентификатор писателя
генерит PHP, как `requestId` у `HttpClient`. Брошенный писатель — корутина умерла, флоу
кончился — закрывается хуком состояния: файл закрывается, недописанный удаляется (кроме
режима Append, по тому же правилу, что `drop_partial`).

Ограничитель разового чтения: payload несёт `maxReadBytes` (аналог `maxResponseBody` у
`HttpClient`). Расширение отказывается читать файл больше лимита вместо того, чтобы
съесть память процесса. Дефолт — 64 МиБ, переопределяется на вызове; `0` снимает лимит.
