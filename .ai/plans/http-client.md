...
Статус: **план** (дизайн до реализации).
...
$client->sendRequest($psrRequest)                    │
  exec(RequestPayload, MethodHttpRequest) ──────────►│ Handle → state.New → states.Start
...
- PHP: `SConcur\Features\MethodEnum::HttpRequest = 5`
- Go: `ext/internal/types/method.go` → `MethodHttpRequest Method = 5`
...
- `state/response.go` — `responseState` (`contracts.StateContract`):
  ...
  - `Next()`:
    ...
    - **последующие вызовы**: `readChunk(bodyReader, chunkSize)` (переиспользовать
      хелпер из `body_state.go` или вынести в общий `internal/httpio`) → сырой
...
**Go:**
- [ ] `types/method.go`: `MethodHttpRequest = 5`.
