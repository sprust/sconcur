...
Статус: **устарел**. Фича реализована (ветка `feature/amqp-rabbitmq`, версия
расширения `0.11.0`), но на той же ветке калька заменена собственным API —
см. [amqp-native-api.md](amqp-native-api.md). Всё, что этот файл говорит про
классы `AMQP*`, флаги-маски и parity по сигнатурам, больше не соответствует
коду; Go-сторона, payload-ы и устройство пулов описаны верно.

Документация — [docs/amqp.ru.md](../../docs/amqp.ru.md); дальше по файлу —
исходный план, он оставлен как есть.
...
- **Ошибка едет с областью и кодом ответа**: полезная нагрузка неуспешной задачи
  — `<scope>:<code>: <текст>`, где scope это `net` / `chn` / `err` (§12 знал
  только маркер `net:`). Без кода не работает `catch (AMQPQueueException $e)` с
  ветвлением по `getCode() === 404`, а это каноничная идиома ext-amqp.
- **Decimal и timestamp едут своими типами поля AMQP**, в тегированной карте
  (§8 исходил из того, что расширение отдаёт их скалярами — это неверно:
  `toAmqpValue()` у обоих возвращает `$this`, и на проводе они кодируются как
  `D` и `T`).
- **Учётные данные не участвуют в URI**: они передаются в `amqp091.Config.SASL`,
  иначе логин или пароль со спецсимволами URI (`%`, `/`, `:`, `#`) не проходит
  аутентификацию.
- **Разделение соединения видно брокеру.** Два объекта `AMQPConnection` с
  одинаковыми учётными данными — это одно соединение (§9), а значит exclusive-
  очередь, объявленная через один объект, доступна через второй. У `ext-amqp`
  это были бы два соединения. Отказаться от разделения можно через
  `connection_name`, он входит в ключ пула.

- **Поля таблицы кодируются по правилам расширения**: нестроковый ключ отбрасывается
  только на верхнем уровне (глубже — приводится к строке уже на стороне Go, потому
  что PHP не умеет держать `"0"` строковым ключом), а вложенный массив без
  строковых ключей едет как AMQP field array, а не как таблица. Первый заход
  отбрасывал ключи на всех уровнях и терял списки в заголовках.
- **Отмена консьюмера сериализована с остальными командами канала**: `basic.cancel`
  идёт через тот же мьютекс, что и всё прочее, а не из сторожевой горутины
  драйвера. Параллельные cancel и channel.close брокер видел как «unexpected
  command» и рвал соединение — в пуле это задевало всех.

Первые пять пунктов — решения по ходу реализации; остальные найдены двумя
раундами ревью после коммитов и внесены в таблицу отклонений `docs/amqp.md` там,
где расхождение осталось осознанным.
