...
- ~~**Rolling reload**~~ (**реализовано**). CLI-команда `reload` (через тот же
  `--configPath`) создаёт триггер-файл `<runtimeDir>/<name>.reload` (без сигнала — нет
  риска PID-reuse, как и у `stop`); мастер замечает его в тике и перекатывает воркеров
  **по одному**: `SIGTERM` → дренаж до `shutdownTimeoutMs` (иначе `SIGKILL`) → свежий
  процесс на замену, полагаясь на ранний выход из `SO_REUSEPORT`-группы для
  zero-downtime при `N>1`. По завершении мастер удаляет триггер, CLI поллит до этого.
  Перезапускает **код** воркеров (не перечитывает конфиг). См.
  `WorkerMaster::driveReload`, `MasterReloadFile`. Конфиг-reload (изменение
  `workerCount`/аргументов на лету) — возможное будущее расширение.
...
- **Watchdog зависших воркеров** (возможно к реализации). Обработчик в нативном
  блокирующем вызове (`sleep`, синхронный PDO/`curl`) или CPU-цикле замораживает
  PHP-поток воркера: `handlerTimeoutMs` отдаёт клиентам `504`, но воркер остаётся
  `running` и выпадает из обслуживания, а мастер этого **не детектит** (см.
  [docs/worker-master.ru.md](../../docs/worker-master.ru.md) «Зависший воркер»).
  Идея: воркер пишет heartbeat (метку времени) из PHP serve-цикла — пока цикл крутится,
  метка свежая; нативный блок/цикл её замораживает. Мастер в своём тике проверяет
  свежесть и при устаревании дольше порога (напр. `handlerTimeoutMs` + запас) делает
  `SIGKILL` + respawn слота. Тонкости: канал heartbeat (файл/пайп) и порог, чтобы
  легитимная async-нагрузка не считалась зависанием; учёт в backoff/`respawnAt`.
