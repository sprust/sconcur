...
- **Watchdog зависших воркеров** (возможно к реализации). Обработчик в нативном
  ...
  Идея: воркер пишет heartbeat (метку времени) **из тика serve-цикла**
  (`Scheduler::serve` опрашивает `waitAnyTimeout` каждые `SERVE_POLL_INTERVAL_MS`
  = 250 мс) — **не** из обработки запроса. Мастер в своём тике проверяет свежесть
  метки и при устаревании дольше порога делает `SIGKILL` + respawn слота.

  **Важно — что НЕ должно считаться зависанием** (метка остаётся свежей, воркер жив):
  - **простой без входящих запросов** — serve-цикл всё равно крутится, опрашивая
    `waitAnyTimeout` каждые 250 мс вхолостую;
  - **долгая легитимная async-работа** в обработчике (Mongo/SQL/`Sleeper`/HTTP-клиент)
    — фибра приостановлена, планировщик продолжает мультиплексировать цикл.

  Зависание — это только **остановка самого цикла**: нативный блок или CPU-спин на
  стеке обработчика, не уступающий планировщику. Поэтому порог привязан к **периоду
  тика serve-цикла** (несколько × 250 мс, т.е. единицы секунд), а **не** к
  `handlerTimeoutMs` — цикл бьётся независимо от длительности запроса. Тонкости: канал
  heartbeat (файл рядом со state-файлом — мастер без расширения читает его `stat`'ом)
  и учёт в backoff/`respawnAt`.
- **Go safety-net для осиротевших воркеров** (возможно к реализации; дополняет
  watchdog). PHP-orphan-чек (`shouldStop` → `posix_getppid() != masterPid`,
  `HttpServer::isOrphaned`) — часть serve-цикла, поэтому **зависший + осиротевший**
  воркер (мастер умер, а PHP-поток заморожен нативным блоком/CPU-циклом) **не**
  самозавершается и держит порт. Master-watchdog тут бессилен — мастер мёртв. Закрыть
  дыру может только проверка **внутри воркера, независимая от PHP-потока**: горутина в
  `serverState` сверяет `os.Getppid()` с `masterPid` (кладём его в `ServePayload`) и при
  сиротстве дольше grace-периода (`> shutdownTimeoutMs` + запас) делает `os.Exit`
  (горутины Go исполняются даже при замороженном PHP — проверено на `handlerTimeoutMs`).
  Это **safety-net, не замена** PHP-пути: в штатном orphan-кейсе раньше срабатывает
  graceful-дренаж в PHP (grace ещё не вышел), а force-exit ловит только зависший случай
  (где дренаж в PHP всё равно невозможен).

  **Полный перенос orphan-логики в Go отвергнут** (вердикт ревью): graceful-дренаж
  in-flight по природе PHP-овый (только PHP дослуживает корутины), вынос детекта
  расщепил бы логику на два слоя и потребовал обратной Go→PHP-координации «начинай
  дренаж» — больше частей ради сценария, который в штатном случае и так работает.
  Детект при этом точнее не становится: `os.Getppid()` == `posix_getppid()`. Связка
  покрытия: мастер жив + воркер завис → master-watchdog; мастер мёртв + воркер завис →
  этот Go-net.
