...
- `Worker` — хелпер для воркер-скрипта: `masterPid()`/`index()`/`count()` (из env).
...
$index = Worker::index(); // из argv-флага --sconcurWorkerIndex (0 вне мастера)
...
$server = new HttpServer(
    ...
    // осиротевших воркеров»). Сервер сверяет posix_getppid() с этим pid.
...
worker (script):
  HttpServer(reusePort: true, maxRequests: N, masterPid: Worker::masterPid())->serve($handler)
...
переиспользованию PID. Знание о значении pid живёт в `Worker::masterPid()` (читает
argv-флаг `--sconcurMasterPid`), а сам сирота-чек — внутри `HttpServer`. `null` (вне
мастера) — проверка выключена. Чистый PHP, протокол не меняется.

> Для не-HTTP воркеров тот же приём: читать `Worker::masterPid()` и периодически
...
- **`reusePort` — обязанность воркера.** Мастер не навязывает его (он универсален),
  ...
  получит `EADDRINUSE`. Мастер передаёт `--sconcurWorkerIndex` через argv — для
  логов, шардирования и джиттера лимитов.
...
- мастер дописывает в `argv` воркера: `--sconcurWorkerIndex` (0..N-1) и
  **`--sconcurMasterPid`** — pid мастера (для сторожа; см. ниже).
...
- pid мастера передаётся argv-флагом `--sconcurMasterPid`; воркер отдаёт его в
  `HttpServer(masterPid:)`, который сверяет `posix_getppid() === masterPid`;
...
2. [x] `WorkerMaster`: спавн N слотов, супервизионный цикл, `RestartPolicy::Always`,
   проброс `phpArgs`/`workerArgs`/`env` + argv-флаги `--sconcurWorkerIndex`/`--sconcurMasterPid`.
...
- **`SIGHUP` rolling reload** (стретч, не сделано) — перезапуск воркеров по одному
  для горячей перезагрузки кода без простоя.
