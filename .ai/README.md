...
- [docs/admin-stats.ru.md](../docs/admin-stats.ru.md) — admin statistics endpoint: per-worker snapshot files, Go-side aggregation across a reuse-port pool, Bearer token, metrics reference
...
- `internal/features/httpserver/` — `net/http.Server` as an http.Handler streaming each request to PHP; response write-commands, request-body streaming, concurrency limit, timeouts, graceful shutdown, SO_REUSEPORT. `stats/` is the admin-statistics subpackage: a per-worker snapshot writer (5s, `<statsDir>/<name>-stats-<pid>.json`) and the aggregator for the reserved `/sconcur-server-api/admin/stats` endpoint (Bearer token, reads sibling snapshots, prunes dead-pid files under flock, flags hung workers). All Go-side — PHP is not involved per admin request. See [docs/admin-stats.ru.md](../docs/admin-stats.ru.md).
