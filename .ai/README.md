...
- `Features/SocketServer/` — long-lived TCP server (message→response over length-prefix framing): `SocketServer::serve()`, `SocketServer::fromArgs()`, DTOs (`Message`/`MessageResponse`), payloads (`ServePayload`/`RespondPayload`). One coroutine per connection loops read-frame→handler→respond; an access log line per connection goes to STDOUT. Shares `Scheduler::serve()` with HttpServer. See [docs/socket-server.ru.md](../docs/socket-server.ru.md).
...
- `internal/features/socketserver/` — raw TCP listener as a streaming state: each accepted connection is one batch streamed to PHP (`ConnectionEvent`); `message_state.go` streams inbound length-prefixed frames (one per `next()`), `server.go` runs the per-connection write loop with write-backpressure and a per-message handler timer (armed when a frame is delivered), `frame.go` is the length-prefix codec, `listen.go` is TCP + `SO_REUSEPORT`. `StopAccepting` closes the listener and half-closes in-flight connections for graceful drain. Two methods, one feature (like httpserver)
