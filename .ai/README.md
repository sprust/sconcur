...
- [docs/cli.md](../docs/cli.md) — console commands: `bin/sconcur-load` (download the matching extension .so), `bin/sconcur-status` (check install/version, `--json`), `bin/sconcur-server` (worker master, brief + link)
- [docs/architecture.md](../docs/architecture.md) — internals: Fiber ↔ goroutine, scheduler, layers, task lifecycle (with Mermaid diagrams)
...
- [docs/worker-master.md](../docs/worker-master.md) — worker master: CLI start/status/stop, restart policy, logging, single-instance, orphan self-termination
- [docs/admin-stats.md](../docs/admin-stats.md) — server statistics: each worker pushes JSON snapshots over a unix socket to a collector embedded in the PHP master, which serves the pool aggregate at GET /api/stats (Prometheus/JSON/HTML), a live panel and SSE, Bearer token, metrics reference
- [docs/http-client.md](../docs/http-client.md) — HTTP-client feature (PSR-18): usage, options, streaming, internals
- [docs/socket-client.md](../docs/socket-client.md) — TCP socket-client feature (dial-side mirror of the socket server): length-prefix framing, Connection read/write/close, params, internals, limits
- [docs/websocket-server.md](../docs/websocket-server.md) — WebSocket-server feature (HTTP-Upgrade listener + socket-server push model): text/binary messages, Connection read/write/close, keepalive ping, params, internals, limits
- [docs/websocket-client.md](../docs/websocket-client.md) — WebSocket-client feature (dial-side mirror of the WebSocket server): connect/read/write/close, text/binary, params, internals, limits
...
- `Context/Context` — static entry point `Context::current(): CoroutineContext` to the current coroutine's context (root outside any fiber); `Context/CoroutineContext` is the framework-neutral `find`/`has`/`set`/`forget` contract. Parent links are recorded in `Scheduler::spawn` / `WaitGroup::add`. See [docs/coroutine-context.md](../docs/coroutine-context.md)
...
languages (see "Bilingual docs" below). They were deliberately reworked to not
read as AI-generated (a contributor flagged the old format). When writing or
editing docs, hold to these rules:
...
- **README is a short "visitor card"**: what it is, for whom, key limits, a quick
  ...
  `docs/architecture.md`), not inline in the README.
...
- **Naming.** English is the base filename (`README.md`, `docs/cli.md`); Russian
  ...
  doc has both files.
- **Edit one → update the other.** Any change to a doc (new section, a fix, a
  reworded paragraph) must be mirrored into the other language in the same commit.
  The two versions never drift. When you fix a factual inaccuracy, fix it in both.
- **Language switcher header.** The first line of every doc is a one-line
  ...
  - English file: `English | [Русский](<name>.ru.md)`
  - Russian file: `[English](<name>.md) | Русский`
- **Internal links follow the file's language.** Links between docs point to the
  same-language pair: English docs link to `*.md`, Russian docs link to `*.ru.md`.
- **English-context references** (this file, `AGENTS.md`, `CLAUDE.md`, source
  comments) link to the English `docs/*.md`.
