...
- [docs/admin-stats.ru.md](../docs/admin-stats.ru.md) — server statistics: a dedicated stats HTTP server (GET /api/stats on its own SO_REUSEPORT port) for HTTP and socket pools, per-worker snapshot files, Go-side aggregation, Bearer token, metrics reference
...
- `internal/stats/` — neutral server-statistics package shared by the HTTP and socket servers: process metrics (`metrics.go`: /proc + runtime), the per-worker snapshot writer (`collector.go`, 5s, `<statsDir>/<name>-stats-<pid>.json`, atomic), the aggregator (`aggregate.go`: reads sibling snapshots, prunes dead-pid files under flock, flags hung workers, sums a `requests` and/or `connections` workload section), and the dedicated stats HTTP server (`server.go`: `GET /api/stats` on its own SO_REUSEPORT port — `listen.go` — Bearer auth, started by `MaybeStartServer` when both a port and a token are set; content-negotiated by `Accept` — `negotiateFormat` picks Prometheus metrics by default (`prometheus.go`), JSON for `application/json`, or HTML for `text/html` (`html.go`)). The feature-specific counters come through a `WorkloadProvider`. All Go-side — PHP is not involved per stats request. See [docs/admin-stats.ru.md](../docs/admin-stats.ru.md).
