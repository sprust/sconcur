...
#
# Usage (from the host):
#   ext-rust/check/fpm-check.sh                     # the Rust core
#   ext-rust/check/fpm-check.sh go                  # the Go core, as the control
set -uo pipefail
...
case "$CORE" in
    rust) EXTENSION=/sconcur/ext-rust/build/sconcur.so ;;
    go)   EXTENSION=/sconcur/ext/build/sconcur.so ;;
...
docker run --rm \
    ...
    bash -c '
        php-fpm -d extension='"$EXTENSION"' \
            -y /sconcur/ext-rust/check/fpm/www.conf >/tmp/fpm.log 2>&1 &
        fpm_pid=$!
        ...
        for _ in $(seq 1 50); do
            if SCRIPT_FILENAME=/sconcur/ext-rust/check/fpm/ping.php \
        ...
        for i in $(seq 1 '"$REQUESTS"'); do
            body=$(
                SCRIPT_FILENAME=/sconcur/ext-rust/check/fpm/index.php \
